...
class LinkedDataHandler:
    ...
    def visualize(self, subset_name, link: dict):
        annotations = link['samples']['lidar']['annotations']
        rgb_path = str(self.root_dir)  + "/" + subset_name + "/samples" + link["samples"]["rgb"]["filename"]
        thermal_path = str(self.root_dir)  + "/" + subset_name + "/samples" + link["samples"]["thermal"]["filename"]
        pcd_path = str(self.root_dir)  + "/" + subset_name + "/samples" + link["samples"]["lidar"]["filename"]
        ...
        for anno in annotations:
            pts_rgb = self.getAnnotationInCameraFrame(anno, link, "rgb")
            pts_thermal = self.getAnnotationInCameraFrame(anno, link, "thermal")
            #draw on image
            color = (0,255,0)
            if 'occluded' in anno.keys() and anno['occluded']:
                color = (255,0,0)
            ...
            if len(pts_rgb) == 8:
                edges = [(0,1),(1,2),(2,3),(3,0),
                        ...
                        (0,4),(1,5),(2,6),(3,7)]
                for i,j in edges:
                    cv2.line(img1, pts_rgb[i], pts_rgb[j], color, 2)

            if len(pts_thermal) == 8:
                edges = [(0,1),(1,2),(2,3),(3,0),
                        (4,5),(5,6),(6,7),(7,4),
                        (0,4),(1,5),(2,6),(3,7)]
                for i,j in edges:
                    cv2.line(img2, pts_thermal[i], pts_thermal[j], color, 2)
        ...
        for i, bbox_data in enumerate(annotations):
            center = np.array(bbox_data["translation"])
            size = np.array(bbox_data["size"])
